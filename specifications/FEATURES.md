...
| CORE-017 | Feature relationship knowledge graph | Show tracked features as a knowledge graph: each feature is a node, and edges show how features relate (depends on, blocks, prerequisites, same category) so a human can see the web of work, not only columns or the Process pipeline | v1 | 🟢 ReadyToMerge | @peterlandis | specifications/plans/CORE-017-FEATURE-KNOWLEDGE-GRAPH-INTENT.md | Tasks: specifications/tasks/CORE-017-IMPLEMENTATION-TASKS.md. Third header view next to Board / Process. Click a node opens the workspace. Voice Jarvis is CORE-018. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/3 |
...
| CORE-019 | Graph zoom and pan | Zoom in on the CORE-017 knowledge graph (scroll wheel, +/−, reset) and pan so feature IDs and titles are readable. Does not start agents | v1 | 🔨 WorkInProgress | @peterlandis | specifications/plans/CORE-019-GRAPH-ZOOM-PAN-INTENT.md | Tasks: specifications/tasks/CORE-019-IMPLEMENTATION-TASKS.md. Depends on CORE-017. |

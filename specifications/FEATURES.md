...
| CORE-018 | Voice Jarvis on the knowledge graph | Hands-off Graph experience: a Jarvis-like virtual person you talk to with voice, that talks back about current state, relations, and what to work on next, using the CORE-017 knowledge graph as its world | v1 | ✅ Complete | @peterlandis | specifications/plans/CORE-018-VOICE-JARVIS-GRAPH-INTENT.md | CORE-017 | Tasks: specifications/tasks/CORE-018-IMPLEMENTATION-TASKS.md. Voice confirm still required before starting agents. Voice picker in Settings → Jarvis (local Mac, browser, OpenAI, or Grok/xAI). OpenAI/Grok voices brief from the graph in conversational speech. Setup follows the chosen voice. Branch: feat/core-018. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/6 |
...
| CORE-020 | Rotating 3D knowledge graph | 3D perspective mode inside the Graph tab. Auto-rotates; grab a feature to stop and inspect. Same zoom as CORE-019. Choose 2D or 3D on the Graph page | v1 | ✅ Complete | @peterlandis | specifications/plans/CORE-020-GRAPH-3D-PERSPECTIVE-INTENT.md | CORE-017 | Tasks: specifications/tasks/CORE-020-IMPLEMENTATION-TASKS.md. 2D/3D switch on Graph. Selection card + related-edge highlight. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/4 |
| CORE-021 | Quiet missing-branch git checks | Stop `git rev-parse` from printing `fatal: Needed a single revision` when CORE-016 looks up a feature branch that does not exist yet | v1 | 🧪 Testing | @peterlandis | specifications/plans/CORE-021-QUIET-GIT-BRANCH-CHECKS-INTENT.md | - | Tasks: specifications/tasks/CORE-021-IMPLEMENTATION-TASKS.md. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/5 |
| CORE-022 | Readable 3D graph labels | Stop the CORE-020 3D graph from stacking every feature ID and title on top of each other so a large board stays readable | v1 | 🔨 WorkInProgress | @peterlandis | specifications/plans/CORE-022-READABLE-3D-GRAPH-LABELS-INTENT.md | CORE-020 | Tasks: specifications/tasks/CORE-022-IMPLEMENTATION-TASKS.md. Labels only for near/hover/selected nodes. Branch: feat/core-022. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/7 |
| CORE-023 | Graph viewport and Jarvis voice fixes | Pin the Graph view to the browser viewport so the network is not scrolled off-screen on tall/fullscreen windows, and make orb replies use the voice chosen in Settings (unlock audio after mic so Autoplay does not fall back to the old browser voice) | v1 | ✅ Complete | @cursor | specifications/plans/CORE-023-GRAPH-VIEWPORT-JARVIS-VOICE-FIX-INTENT.md | - | Tasks: specifications/tasks/CORE-023-IMPLEMENTATION-TASKS.md. Bugfix on Graph + Jarvis. Branch: feat/core-023. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/8 |
| CORE-024 | 3D graph titles and default mode | Show feature titles (not only IDs) on the 3D knowledge graph, and open Graph in 3D by default | v1 | ✅ Complete | @cursor | specifications/plans/CORE-024-3D-TITLES-DEFAULT-INTENT.md | CORE-020, CORE-022 | Tasks: specifications/tasks/CORE-024-IMPLEMENTATION-TASKS.md. Merged to main |
| CORE-025 | Per-feature Cursor model | Choose which Cursor model to use when starting planning, revision, or implementation for a feature, instead of only the global Settings default | v1 | ✅ Complete | @cursor | specifications/plans/CORE-025-PER-FEATURE-CURSOR-MODEL-INTENT.md | AGENT-001, CORE-008 | Tasks: specifications/tasks/CORE-025-IMPLEMENTATION-TASKS.md. Merged to main |
| CORE-026 | Workspace opens without git repo | Opening a feature workspace must not crash when FEATURES.md lives outside a git repository; Ship should explain that git is missing instead of throwing | v1 | ✅ Complete | @cursor | specifications/plans/CORE-026-WORKSPACE-WITHOUT-GIT-INTENT.md | - | Tasks: specifications/tasks/CORE-026-IMPLEMENTATION-TASKS.md. Merged to main |
| CORE-027 | 3D graph in-view feature roster | While the 3D graph spins, show a left-hand list of features currently in the viewport (as many as fit the window) with brief id/title/description so you can read what is on screen without relying only on node labels | v1 | ✅ Complete | @cursor | specifications/plans/CORE-027-3D-IN-VIEW-ROSTER-INTENT.md | CORE-020, CORE-024 | Tasks: specifications/tasks/CORE-027-IMPLEMENTATION-TASKS.md. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/11 |
| CORE-028 | Graph category clusters and dependency suggestions | Show category groupings as visual clusters on the Graph (not only sparse same-category edges), recognize project feature IDs beyond CORE/AGENT, and suggest Depends-on links you can apply so related work is visible and prioritizable | v1 | ✅ Complete | @cursor | specifications/plans/CORE-028-GRAPH-CLUSTERS-DEPENDENCY-SUGGESTIONS-INTENT.md | CORE-017 | Tasks: specifications/tasks/CORE-028-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/12 |
| CORE-029 | Graph related-features roster | When you highlight a feature on 2D (or pin one on 3D), show a left-hand Related list with id, title, short description, and relation—same readable detail in both modes—while 3D keeps In view while spinning | v1 | ✅ Complete | @cursor | specifications/plans/CORE-029-2D-RELATED-ROSTER-INTENT.md | CORE-017, CORE-027 | Tasks: specifications/tasks/CORE-029-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/13 |
| CORE-030 | Suggested focus (where to start) | On Process, show Start here and Next features ranked from graph deps and human gates; reuse the same ranking for Jarvis “what next,” and deepen by focusing the pick on Graph. Board stays drag-and-drop only | v1 | ✅ Complete | @cursor | specifications/plans/CORE-030-SUGGESTED-FOCUS-INTENT.md | CORE-017, CORE-018, CORE-029 | Tasks: specifications/tasks/CORE-030-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/14 |
| CORE-031 | Phased overnight feature runner | On Process, build a phased plan from Suggested focus and Run phase to sequentially plan (and optionally implement with batch plan auto-approve) each feature with artifacts committed to git, so you can wake up and review a finished phase. Board stays drag-and-drop only | v1 | ✅ Complete | @cursor | specifications/plans/CORE-031-PHASED-OVERNIGHT-RUNNER-INTENT.md | AGENT-001, CORE-030 | Tasks: specifications/tasks/CORE-031-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/15 |
| CORE-032 | Auto-inflate specifications layout | When opening or creating a project FEATURES.md for the first time, create the missing spec-driven folder structure (plans/tasks/reviews/completions/templates) and template files if they are absent, so agents can plan without a manual inflate skill | v1 | ✅ Complete | @cursor | specifications/plans/CORE-032-AUTO-INFLATE-SPECS-INTENT.md | CORE-006 | Tasks: specifications/tasks/CORE-032-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/16 |
| CORE-033 | Depends column standard | Make FEATURES.md dependencies a first-class Depends column (not Notes), migrate existing Depends-on notes automatically, and AI-repair malformed trackers when a chat key is configured | v1 | 🔨 WorkInProgress | @cursor | specifications/plans/CORE-033-DEPENDS-COLUMN-STANDARD-INTENT.md | CORE-017, CORE-028, CORE-032 | Tasks: specifications/tasks/CORE-033-IMPLEMENTATION-TASKS.md. Branch: feat/core-033. |
| CORE-034 | Phase live CoT and stage models | While a phase runs, show live agent CoT on Process, pick Cursor models per stage (plan/implement/review), auto-move phase features across Process stage groups, and draw dependency lines between them so phase progress is visible | v1 | ✅ Complete | @cursor | specifications/plans/CORE-034-PHASE-COT-STAGE-MODELS-INTENT.md | CORE-011, CORE-025, CORE-031 | Tasks: specifications/tasks/CORE-034-IMPLEMENTATION-TASKS.md. Merged to main. · PR: https://github.com/peterlandis/spec-feature-kanban/pull/17 |
...
| AGENT-002 | Hosted phone-reachable service | Host the control plane over HTTPS so any device can kick off remote/cloud agents against a connected git repo and get notified at human gates | Later | 📋 Planned | - | - | AGENT-001 | Not v1 |

...
| CORE-023 | Graph viewport and Jarvis voice fixes | Pin the Graph view to the browser viewport so the network is not scrolled off-screen on tall/fullscreen windows, and make orb replies use the voice chosen in Settings (unlock audio after mic so Autoplay does not fall back to the old browser voice) | v1 | 🔨 WorkInProgress | @cursor | specifications/plans/CORE-023-GRAPH-VIEWPORT-JARVIS-VOICE-FIX-INTENT.md | Tasks: specifications/tasks/CORE-023-IMPLEMENTATION-TASKS.md. Bugfix on Graph + Jarvis. Branch: feat/core-023. |
| CORE-026 | Workspace opens without git repo | Opening a feature workspace must not crash when FEATURES.md lives outside a git repository; Ship should explain that git is missing instead of throwing | v1 | 🔨 WorkInProgress | @cursor | specifications/plans/CORE-026-WORKSPACE-WITHOUT-GIT-INTENT.md | Tasks: specifications/tasks/CORE-026-IMPLEMENTATION-TASKS.md. Branch: feat/core-026. |
